...
<template>
  ...
  <div class="md:px-32 px-6  md:my-24 my-12">
    <div class="flex flex-col gap-8">
      <div class="grid md:grid-cols-6 grid-cols-1 mb-12 gap-6" v-for="(serv,index) in services" :key="serv._id" :id="serv._id">
        <div class="serv-img md:col-span-2 grid-cols-1" :class="[(index + 1) % 2 !== 0 ? 'order-1':'']" data-aos="fade-right">
            <img :src="serv.image" class="md:h-80" alt="">
        </div>
        <div class="serv-content flex flex-col md:col-span-4 grid-cols-1" :data-aos="(index+1) % 2 !== 0 ? 'fade-right':'fade-left' ">
          <h2 class="md:text-4xl text-2x1 text-primary font-semibold">{{serv.header}}</h2>
          <ul class="list-disc ml-8 mt-6 flex flex-col gap-2 md:text-xl text-sm">
